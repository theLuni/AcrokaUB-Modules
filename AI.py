"""
Описание: Модуль для запросов к нейросети через API
"""
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(state.api_url, json=payload) as response:
                    ...
                    # Всегда пытаемся декодировать base64
                    if isinstance(answer, str):
                        try:
                            # Декодируем и заменяем содержимое
                            decoded = base64.b64decode(answer).decode('utf-8')
                            answer = decoded
                        except:
                            # Оставляем оригинальный ответ если декодирование не удалось
                            pass
